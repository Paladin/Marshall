...
license. For the sake of consistency, this is also licnsed under th apache license.

==Why another one?

Basically, because I wanted a different approach to the output. I wanted semantically proper markup, and I wanted to control the appearance of everything with css, rather than inline styles written (and controlled) by javascript. I recognize this is a different goal from the original, basically when the library reaches a release stage, the only thin that will remain of the original code is the pgn engine and maybe some of the structure. It was to much to ask that the original author accept changes that would quite possibly break the WP plugin he also offered, so a fork was the kindest option for all.
...
Simple. The main tree is the development versions, so each class is in its own file. When the releases start to happen, there will be a separate dircetory for the combined and minified versions of the javascript; use those.

For now, each class/function is on its own. Deal with it.
